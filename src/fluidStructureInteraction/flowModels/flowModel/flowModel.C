...
namespace Foam
{
    defineTypeNameAndDebug(flowModel, 0);
    defineRunTimeSelectionTable(flowModel, dictionary);
}

// * * * * * * * * * * * * * Private Member Functions  * * * * * * * * * * * //

void Foam::flowModel::calcGlobalFaceZones() const
{
    ...
    if (globalFaceZonesPtr_)
    {
        FatalErrorIn
        (
            "void flowModel::calcGlobalFaceZones() const"
        )
...
void Foam::flowModel::calcGlobalToLocalFaceZonePointMap() const
{
    // Find global face zones
    if (globalToLocalFaceZonePointMapPtr_)
    {
        FatalErrorIn
        (
            "void flowModel::calcGlobalToLocalFaceZonePointMap() const"
        )
...
//         }
        ...
        forAll(curMap, globalPointI)
        {
            if (curMap[globalPointI] == -1)
            {
                FatalErrorIn
                (
                    "flowModel::calcGlobalToLocalFaceZonePointMap()"
                )
...
Foam::flowModel::globalFaceZones() const
{
    if (!globalFaceZonesPtr_)
    {
        calcGlobalFaceZones();
...
Foam::flowModel::globalToLocalFaceZonePointMap() const
{
    if (!globalToLocalFaceZonePointMapPtr_)
    {
